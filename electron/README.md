...
- `runtime-environment.cjs`, `managed-projects.cjs`, `update-service.cjs`: packaged Python/Git paths, staged sparse workspace preparation, conservative managed `main` updates and a bounded pre-editor application update gate. Only the fixed project origins and release page cross these new IPC operations. Existing contributor folders are never adopted for automatic updates.
...
`npm run dist` builds native installers with a pinned Python/Git runtime. Python service resources remain ordinary files (`asar: false`); authoring transforms are precompiled into `electron/authoring/shared-bundle.cjs` so packaged apps need no runtime TypeScript compiler. Development still compiles from current source. See [installation and releases](../docs/installing.md); the [recorded dependency instructions](../docs/design/dependencies.md) describe the historical developer baseline.
