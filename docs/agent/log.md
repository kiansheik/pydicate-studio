...
Because every returned candidate realizes the observation, a generating expression cannot show the alternatives are wrong, so only contributor-decided pairs are trained on and undecided ones are counted as skipped. That makes corrections the real signal, and the laboratory now keeps them: every analysis, every verdict with the whole set that was on screen and which reading was chosen, and whether a correction had been proposed at all — separating coverage failures from ranking ones. Choosing one of the readings on screen counts as a preference over the others — passed over, not wrong, and weaker than an explicit rejection — so one click already yields a usable contrast. A decision applies to the next analysis immediately, before any training. Confirmed and corrected readings become reviewed examples on real input that populate the previously empty historical suite, and unanalysable inputs become a coverage-gap list naming the pieces that were recognized. Scoring reports exact, annotation-identical and any-valid separately rather than conflating them.

The smoke profile gained `apé` so the smallest index exercises a real ambiguity. Fixed a confirmation message that the refresh behind it wiped. 66 focused Python laboratory checks, 147 renderer, 197 desktop and 12 browser scenarios pass; the native Electron proof still passes all eleven stages. No provider call, no publication, no approval. See [contract](../design/parser-lab.md).
