...
Tupi → Pydicate laboratory (hidden, experimental): `src/components/ParserLab.tsx`, `src/domain/parser-lab.ts`, `src/parser-lab.css` host the lazy workspace and the shared tree editor; the switch lives in App's project information panel. `electron/parser-lab-service.cjs` owns artifact-scoped job records, a lazily started laboratory worker and real cancellation, reached through `next-service.cjs` by the `parser_lab_` prefix. `python/parser_lab/` holds the answer-free engine binding, the `lab-v1` input profile, the declared grammar families, the bounded retrieve/compose/rank/validate cascade, the source-AST projection, artifact manifests, grouped splits, the trainable ranker, the evaluation suites and the optional neural/agent adapters, with `worker.py` as its persistent process. `scripts/parser-lab/cli.py` and `configs/parser-lab/` make every operation scriptable; `scripts/smoke-parser-lab.mjs` is the native proof. See [the contract](../design/parser-lab.md), `python/tests/test_parser_lab.py`, `electron/tests/parser-lab-service.test.cjs`, `src/domain/parser-lab.test.ts`, `tests/parser-lab.spec.ts` and `tests/parser-lab-shell.spec.ts`.
